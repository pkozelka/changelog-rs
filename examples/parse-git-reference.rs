...
impl GitLogReferenceLine {
    fn parse(line: &str) -> Result<Self> {
        let inner_start = line.find(" (").ok_or(Error::new(
            ErrorKind::InvalidData,
            format!("Cannot find hash separator: {}", line),
        ))?;
        if !line.ends_with(")") {
            return Err(Error::new(
                ErrorKind::InvalidData,
                format!("Cannot find right brace: {}", line),
            ));
        }

        let inner = &line[inner_start + 2..line.len() - 2];
        let text_end = inner.rfind(", 2").ok_or(Error::new(
            ErrorKind::InvalidData,
            format!("Cannot find timestamp separator: {}", line),
        ))?;
